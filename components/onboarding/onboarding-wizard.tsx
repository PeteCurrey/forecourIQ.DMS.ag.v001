...
export default function OnboardingWizard({
  ...
  // Keep a mutable copy of the dealership so all steps see updates
  const [dealershipState, setDealershipState] = useState<any>(dealership)
  const [step, setStep] = useState(Number(searchParams.get('step')) || 1)

  // Advance to the next step, optionally updating the dealership in state
  const handleNext = (updatedDealership?: any) => {
    if (updatedDealership) setDealershipState(updatedDealership)
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <header className="px-12 py-8 flex justify-between items-center shrink-0">
        ...
        <div className="flex items-center gap-8">
          <span className="font-mono text-[11px] text-pewter uppercase tracking-[0.15em]">
            STEP {step} OF 5
          </span>
          <button
      ...
      <div className="flex-1 flex flex-col justify-center px-12 pb-20">
        <div className="max-w-4xl mx-auto w-full">
          ...
          {step === 5 && (
            <StepComplete dealership={dealershipState} />
          )}
